...
function MorphingDialogTrigger({ children, className, style, triggerRef }: MorphingDialogTriggerProps) {
	...
	return (
		<motion.button
			...
			className={cn('relative cursor-pointer rounded-[20px]!', className)}
			onClick={handleClick}
			...
			{isOpen && (
				<>
					<motion.div
						...
						className='fixed inset-0 h-full w-full backdrop-blur-xs z-200'
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
					/>
					<div className='fixed inset-0 z-200 flex items-center justify-center'>{children}</div>
				</>
			)}
		</AnimatePresence>,
		document.body
...
function MorphingDialogImage({ src, alt, className, style, ref, onClick }: MorphingDialogImageProps) {
	const { uniqueId } = useMorphingDialog()
...
function MorphingDialogClose({ children, className, variants }: MorphingDialogCloseProps) {
	...
	return (
		<motion.button
			...
			className={cn('absolute top-6 right-6 z-900', className)}
			initial='initial'
			animate='animate'
			exit='exit'
			variants={variants}
